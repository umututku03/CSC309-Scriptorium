...
import { verifyAuthorizationHeader } from "@/utils/auth"; // Import your authentication utility
  ...
  try {
    ...
    try {
      ...
      if (language === "c" || language === "cpp") {
        await fs
          .stat(`${filePath}.out`)
          .then(() => fs.unlink(`${filePath}.out`))
          .catch(() => {});
      }
      if (language === "java") {
        await fs
          .stat(path.join(projectTmpDir, `${className}.class`))
          .then(() => fs.unlink(path.join(projectTmpDir, `${className}.class`)))
          .catch(() => {});
      }
      if (stdin) await fs.unlink(inputPath);
      await fs
        .stat(outputPath)
        .then(() => fs.unlink(outputPath))
        .catch(() => {});
    } catch (cleanupError) {
